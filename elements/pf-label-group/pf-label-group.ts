 ...
 */
const REMAINING_RE = /\$\{\sremaining\s\}/g;
...
export class PfLabelGroup extends LitElement {
  ...
  */
  ...
  private labelColors = ['grey', 'blue', 'green', 'orange', 'red', 'purple', 'cyan', 'gold'];
  ...
  #setNewLabelColor(color: string) {
    this.newLabelColor = color;
    this.requestUpdate();
  }


  #setNewLabelIcon(icon: string) {
    this.newLabelIcon = icon;
    this.requestUpdate();
  }
  ...
  @query('#modal') private _modal?: HTMLDivElement;
  @query('#custom-label-modal')
  private _customModal!: HTMLElement & { show: () => void; close: () => void };

  @queryAssignedNodes({ slot: 'category', flatten: true })
      ...
      <div id="outer" class="${classMap({ 'has-category': hasCategory, empty })}" role="toolbar">
        ...
        ${this.addLabelMode !== 'none' ?
          ...
          <div style="position: relative; display: inline-block;">
            ...
            html`
                <div
                  style="position: absolute; top: 100%; left: 0; z-index: 100%; background: white; border: 1px solid #ccc; box-shadow: 0 2px 6px rgba(0,0,0,0.15);"
                >
                          ...
                          html`
                              <section>
                                <pf-modal
                                  ...
                                  aria-describedby="custom-label-modal-description"
                                  variant="small"
                            @close=${this.#closeCustomForm}
                                >
                                  <!-- Header -->
                                  ...
                                  <div id="add-label-form">
                                    <label>
                                      ...
                                      <input
                                        ...
                                        @input=${(e: Event) =>
                            (this.newLabelText = (e.target as HTMLInputElement).value)}
                                      />
                                    </label>

                         <label>Color</label>

                              <pf-select id="color-select"
                                @change=${this.#onSelectColor} 
                                style="
                                  display: inline-block;
                                  --pf-c-button--PaddingLeft: 0.5rem;
                                  --pf-c-button--PaddingRight: 0.5rem;
                                  --pf-c-button--MinWidth: 0;
                                ">
                                ...
                                <pf-option value="Purple">Purple</pf-option>
                                    ...
                                    <label>Icon</label>
                                      ...
                                      <pf-select id="icon-select"
                                @change=${this.#onSelectIcon} 
                                style="
                                  display: inline-block;
                                  --pf-c-button--PaddingLeft: 0.5rem;
                                  --pf-c-button--PaddingRight: 0.5rem;
                                                    ...
                                                    <div class="radio-group" id="label-type-group">
                                                      <label>
                                                        <input
                                                          ...
                                                          name="labelType"
                                                          value="filled"
                                                          .checked=${this.newLabelVariant === 'filled'}
                                                          @change=${() => (this.newLabelVariant = 'filled')}
                                                        />
                                                        <span class="radio-text">Filled</span>
                                                      </label>
                                                      <label>
                                                        <input
                                                          type="radio"
                                                          name="labelType"
                                                          value="outline"
                                                          .checked=${this.newLabelVariant === 'outline'}
                                                          @change=${() => (this.newLabelVariant = 'outline')}
                                                        />
                                                        <span class="radio-text">Outline</span>
                                                      </label>
                                                    </div>
                                                    ...
                                                    <div class="radio-group" id="dismissable-group">
                                                      <label>
                                                        <input
                                                          ...
                                                          name="dismissable"
                                                          value="true"
                                                          .checked=${this.newLabelDismissable === true}
                                                          @change=${() => (this.newLabelDismissable = true)}
                                                        />
                                                      ...
                                                      <label>
                                                        <input
                                                          ...
                                                          name="dismissable"
                                                          value="false"
                                                          .checked=${this.newLabelDismissable === false}
                                                          @change=${() => (this.newLabelDismissable = false)}
                                                        />
  ...
  #onSelectColor(e: Event) {
    ...
    this.newLabelColor = selectedValue;
    this.#setNewLabelColor(this.newLabelColor.toLowerCase());
  }

  // #onSelectColor(e: Event) {
  //   const select = e.target as any; // PfSelect
  //   const value = select.value; // הערך שנבחר
  //   this.newLabelColor = value as LabelColor;

  //   // למצוא את הטקסט שמוצג מה־pf-option לפי הערך
  //   const option = Array.from(select.querySelectorAll('pf-option'))
  //     .find((opt: any) => opt.value === value);

  //   if (option) {
  //     select.label = option.textContent ?? ''; // זה מה שמוצג למשתמש
  //   }

  //   this.#setNewLabelColor(this.newLabelColor);
  // }
  ...
  #onSelectIcon(e: Event) {
    const select = e.target as HTMLSelectElement;
    const selectedValue = select.value;
    if (selectedValue === 'No icon') {
      this.newLabelIcon = '';
    } else {
      this.newLabelIcon = selectedValue.toLowerCase().concat('-circle');
    }
    this.#setNewLabelIcon(this.newLabelIcon);
  }


  /** Updates overflow state */
  #updateOverflow() {
